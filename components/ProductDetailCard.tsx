      ...
      <div className="grid bg-white shadow-lg rounded p-3 grid-cols-2 sm:grid-cols-1">
        <div className="p-4 flex flex-row-reverse justify-between lg:justify-center lg:flex-col">
          <div className="w-full">
            <img src={imagen} alt="Producto" className="rounded" />
          </div>
          <div className="flex flex-col justify-evenly mt-2 p-4 lg:p-0 lg:flex-row">
            {[0, 1, 2].map((p, idx) => (
              <div
                onMouseMove={() =>
                  setSmallImages({ ...smallsImages, [idx]: styleOn })
                }
        ...
        <div className="flex flex-col p-4">
          <div className="h-64 flex flex-col justify-between">
            ...
            {!promo && (
              <p className="flex rounded w-max p-1 text-white bg-green-800 text-sm">
                Oferta del día
              </p>
            )}
            <p className="flex">
              {stock > 1 ? "Stock disponible" : "¡Último en stock!"}
            </p>
            <p className="text-sm text-green-900 font-medium">
              Pagá con tarjeta de débito/crédito o efectivo
            </p>
          </div>
          <br />
          <div className="h-60 flex flex-col justify-between">
            <button className="border border-green-900 p-2 text-green-900 rounded hover:opacity-95">
              Agregar al carrito
            </button>
            <button className="border p-2 bg-green-800 text-white rounded hover:opacity-95">
              Comprar ahora
            </button>
            <div className="flex justify-between flex-col h-28">
              <p className="text-green-900 text-sm font-medium hover:underline cursor-pointer">
                Envío a destino | Ver zona de entrega
              </p>
              <p className="text-sm hover:underline cursor-pointer">
                ¿Dudas? Contacta a un vendedor
              </p>
              <p className="text-sm hover:underline cursor-pointer">
                Venta telefónica o por whatsapp
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-6 p-4">
            {[0, 1, 2, 3].map((p) => (
              <ProductCard
                ...
                product={{
                  promo: true,
                  _id: "617b007670e4c2107d188aad",
                  nombre: "Proteina de arvejas vegana Granger 1kg",
                  stock: 20,
                  categoriaId: "5",
                  descripcion:
                    "Aporta proteínas que favorecen la recuperación muscular y el aumento de masa muscular.",
                  precio: 200,
                  imagen:
                    "https://siempresano.com.ar:3009/api/containers/images/download/aritos de fruta.jpg",
                }}
              />
            ))}
          </div>
